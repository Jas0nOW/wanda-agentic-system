...
class WandaOrb:
    ...
    def _on_draw(self, widget, cr):
        ...
        if self.state == "speaking":
            for i in range(3):
                ...
                alpha = 0.3 - i * 0.1
                cr.set_source_rgba(self.current_r, self.current_g, self.current_b, alpha)
                cr.set_line_width(2)
                cr.arc(cx, cy, wave_r, 0, 2 * math.pi)
                cr.stroke()
        
        # Main orb with gradient
        gradient = cairo.RadialGradient(cx - radius/3, cy - radius/3, 0, cx, cy, radius)
        gradient.add_color_stop_rgba(0, glow[0], glow[1], glow[2], 1.0)
        gradient.add_color_stop_rgba(0.5, self.current_r, self.current_g, self.current_b, 0.95)
        ...
        # Glass highlight
        highlight = cairo.LinearGradient(cx - radius, cy - radius, cx + radius, cy + radius)
    ...
    def _on_button_release(self, widget, event):
        ...
        if event.button == 1 and self.press_pos:
            ...
            if dx < self.DRAG_THRESHOLD and dy < self.DRAG_THRESHOLD and not self.is_dragging:
                if self.on_click:
    ...
    def _on_motion(self, widget, event):
        ...
        if self.press_pos:
            ...
            if dx > self.DRAG_THRESHOLD or dy > self.DRAG_THRESHOLD or elapsed > self.HOLD_THRESHOLD_MS:
...
if __name__ == "__main__":
    if GTK_AVAILABLE:
        ...
        # Demo state changes
        def demo():
            states = ["listening", "thinking", "speaking", "autonomous", "paused", "idle"]
            for state in states:
                time.sleep(3)
                print(f"State: {state}")
